...
int  main(){
	...
	*****************************/
	...
	#endif
	#if 1
	ofstream file;file.open("015937_select_100_150zen_30_50.txt");
		...
		if(knhit<100||nmuon<150||zen*57.296<40)continue;
		while(i<wNum){
			wcdae->GetEntry(i);
				...
				getHitMjd(second,low_th,coarse,&hitt);
					...
					if (fabs(deltat)<3e-6){
						...
						getCharge(anode,dynode,spe[ig],da[ig],&npe);
                                        	if(j==0){
						file<<setprecision(16)<<km2ati<<"  "<<xc<<"  "<<yc<<"  "<<deltat<<"  "<<npe
						<<"  "<<zen*57.296<<"  "<<nmuon<<"  "<<knhit<<"  "<<j<<endl;
						}
                                	}
