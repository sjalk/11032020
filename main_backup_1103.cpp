...
int  main(){
	...
	*****************************/
	...
	#endif
	#if 1
	ofstream file;file.open("015937_select_highenergy_100_150_100m.txt");
		...
		azi = km2a->r_Phi;
		nmuon = km2a->NuM4;
		km2ati = (kMjd-int(kMjd))*86400+5.43e-6;
		if(nmuon<=150||knhit<100)continue;
			...
			for(int j = 0;j<wcdae->nhit;j++){
				...
				getHitMjd(second,low_th,coarse,&hitt);
					...
					if (fabs(deltat)<3e-6){
						...
						mid = int(wcdae->nhit/2);
                        			second = wcdae->hits_second[mid];
                        			low_th = wcdae->hits_low_th_fine_time[mid];
                        			coarse = wcdae->hits_coarse_time[mid];
                        			getHitMjd(second,low_th,coarse,&hitmid);
						fee = wcdae->hits_fee[mid];
                                        	ch = wcdae->hits_ch[mid];
                                        	iconf = dc::findconf(fee,ch);
                                        	position_x = wcdaconf[iconf][2];
                                        	position_y = wcdaconf[iconf][3];
                                        	coreChange(position_x,position_y,&xck,&yck);
                                        	checkNearShower(xck,yck,hitmid,xc,yc,km2ati,zen,azi,&deltatmid,&ri);
						deltatnew = deltat - deltatmid;
						...
						//miucount=1;
						if(sqrt((xck-xc)*(xck-xc)+(yck-yc)*(yck-yc))>100){
                                		file<<setprecision(16)<<km2ati<<"  "<<hitt<<"  "<<deltat<<"  "<<deltatnew<<"  "<<npe
						<<"  "<<i<<"  "<<k<<"  "<<j<<endl;
						}
					}
		...
		/*
			if(miucount/wcdae->nhit>0&&gamma==1){
				file<<setprecision(16)<<xc<<"  "<<yc<<"  "<<zen<<"  "<<azi<<"  "<<gamma
				<<"  "<<nmuon<<"  "<<knhit<<"  "<<num<<"  "<<npe3<<"  "<<k<<"  "<<i<<endl;
				miucount=0;
			}
			*/
			//miucount=0;
